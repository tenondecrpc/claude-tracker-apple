...
- [x] 1.3 Create `StatsDetailView` in `ClaudeTracker macOS/StatsDetailView.swift` receiving `MacAppCoordinator`
...
- [x] 1.10 Add `Window("Stats", id: "stats-detail")` scene in `ClaudeTrackerMacApp.swift` with `.windowResizability(.contentSize)`
...
- [x] 2.1 Create `UsageSnapshot` struct in `ClaudeTracker macOS/UsageHistory.swift` (date, utilization5h, utilization7d)
- [x] 2.2 Create `UsageHistory` Observable class: load/save JSON at `~/.config/claude-tracker/usage-history.json`, append snapshot, prune entries older than 30 days
...
- [x] 3.1 Create `ClaudeLocalDBReader` in `ClaudeTracker macOS/` — async read of `~/.claude/` session DB, decode sessions with timestamps, project names, token counts per model, tool calls, cost
